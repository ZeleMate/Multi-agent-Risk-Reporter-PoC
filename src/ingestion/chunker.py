    ...
    def create_chunks_from_thread(self, thread_data: dict[str, Any]) -> list[Chunk]:
        ...
        for _i, sentence in enumerate(sentences):
            ...
            if current_tokens + sentence_tokens > self.chunk_size and current_chunk:
                ...
                chunk_start_pos = sum(
                    len(" ".join(sentences[:current_chunk_start])) + 1
                ...
                overlap_start_pos = sum(
                    len(" ".join(sentences[: len(current_chunk) - len(overlap_sentences)])) + 1
        ...
        if current_chunk:
            ...
            final_chunk_start_pos = sum(
                len(" ".join(sentences[: len(sentences) - len(current_chunk)])) + 1
